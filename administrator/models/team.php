...
class DJLeagueModelTeam extends DJLeagueModelAdmin
{
	protected $text_prefix = 'COM_DJLEAGUE';
	protected $form_name = 'team';

	public function __construct($config = array()) {
		parent::__construct($config);
	}

	public function getTable($type = 'Teams', $prefix = 'DJLeagueTable', $config = array())
	{
	...
	public function delete(&$cid) {
		if (count( $cid ))
		{
			...
			$this->_db->setQuery("SELECT COUNT(*) FROM #__djl_tables WHERE team_id IN ( ".$cids." )");
			if ($this->_db->loadResult() > 0) {
				$this->setError(JText::_('COM_DJLEAGUE_ERROR_TEAM_ASSIGNED_TO_LEAGUE'));
				return false;
			}
